...
**Kazanımlar:**
- Tüm kavramları tek bir production-grade projede birleştirme
- Multi-agent orkestrasyon ile gerçek iş akışı
- GraphRAG hafıza sistemi
- Model routing ve maliyet optimizasyonu
- Production-ready mimari (retry, timeout, structured logs)

**Proje:** TwinGraph Studio — Agentic Content & Research Orchestrator
- 5 farklı agent rolü (Research, Writing, Reflection, Repurpose, Cost Guard)
- Deep Research + GraphRAG hafıza ile içerik üretimi
- Medium makalesi → LinkedIn postu dönüşüm pipeline'ı
- Eval sistemi (writing coherence, citation coverage, token efficiency)
- Akıllı model routing (ucuz model → planlama, pahalı model → final yazım)
